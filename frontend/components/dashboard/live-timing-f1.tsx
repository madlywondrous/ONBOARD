...
export const LiveTimingF1 = React.memo(function LiveTimingF1() {
  ...
  // CRITICAL FIX: Track previous data to force updates even when object reference is same
  const prevSseDataRef = useRef<string | null>(null)
  
  // Handle SSE data updates - prioritize fast UI update
  useEffect(() => {
    if (!sseData) {
      return
    }
    
    // CRITICAL: Use last_update timestamp to detect changes, not object reference
    // This ensures we process updates even when React thinks the object is the same
    const currentUpdateKey = String(sseData.last_update || sseData._updateTimestamp || JSON.stringify(sseData))
    
    // Skip if this exact update was already processed
    if (prevSseDataRef.current === currentUpdateKey) {
      return
    }
    
    prevSseDataRef.current = currentUpdateKey
    
    // CRITICAL: Always process updates, even if they appear unchanged
    // This ensures frontend gets updates when nested values change
    if (state.loading) {
      dispatch({ type: 'SET_LOADING', payload: false })
    }
    // Process data update - always dispatch to ensure React detects changes
    dispatch({ type: 'UPDATE_FROM_WEBSOCKET', payload: sseData })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sseData]) // CRITICAL: Only depend on sseData, not state values
