...
@app.get("/api/live/audio-streams")
async def get_audio_streams():
    """Expose audio stream metadata for debugging and proxy usage"""
    try:
        streams = await f1_client.get_audio_streams()
...
@app.get("/api/live/content-streams")
async def get_content_streams():
    """Expose content stream metadata for debugging and proxy usage"""
    try:
        content_streams = await f1_client.get_content_streams()
        return content_streams if content_streams else {}
    except Exception as e:
        logger.error(f"Error getting content streams: {e}")
        return {}


@app.get("/api/debug/client-headers")
async def get_client_headers():
    """Debug helper to inspect client headers used for upstream requests."""
    try:
        return f1_client.headers
    except Exception as e:
        logger.error(f"Error getting client headers: {e}")
        return {}
...
async def get_driver_standings():
    ...
    return await get_driver_standings_data()


@app.get("/api/standings/constructors")
async def get_constructor_standings():
    """Get constructor championship standings"""
    return await get_constructor_standings_data()
